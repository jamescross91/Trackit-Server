	...
	public Representation acceptItem(Representation entity) {
		...
		if (thisDevice.authenticateToken(auth_token)) {
			...
			if (convexMarkerLists != null) {
				...
				try {
					for (Entry<String, HashMap<String, ConvexHullPoint>> groupEntry : convexMarkerLists
							.entrySet()) {
						JSONObject thisGroup = new JSONObject();
						String group_id = groupEntry.getKey();
						HashMap<String, ConvexHullPoint> group = groupEntry
								.getValue();

						for (Entry<String, ConvexHullPoint> markerEntry : group
								.entrySet()) {
